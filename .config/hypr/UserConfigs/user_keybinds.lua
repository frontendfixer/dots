...
local overrides = {
    ...
    { key = mainMod .. " + B", action = hl.dsp.exec_cmd("google-chrome") },
    { key = mainMod .. " + SHIFT + B", action = hl.dsp.exec_cmd("firefox") },
    { key = mainMod .. " + SHIFT + G", action = hl.dsp.exec_cmd("flameshot gui") },
    { key = mainMod .. " + P", action = hl.dsp.exec_cmd("pkill rofi || rofi -modi drun -show drun -config ~/.config/rofi/col_singlerow.rasi") },
